...
2. `CustomerController` accepts `CustomerRequest` — TODO: what does presentation own? The presentation owns the HTTP contract 
3. `CustomerService` applies business rules — TODO: list 1–2 rules (unique ID, status default) Each customer must havea unique ID New customers must default to some sort of status 
