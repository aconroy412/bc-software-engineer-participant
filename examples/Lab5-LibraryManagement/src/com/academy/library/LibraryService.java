...
public class LibraryService {
    ...
    public void borrowBook() {
        ...
        String bookId = "";
        boolean valid = true;
        while(bookId.isBlank() || bookId.isEmpty() || !valid) {
            if (!valid) {
                System.out.println("That book ID does not exist in our records.");
            }
            System.out.print("Hello, what is the book id that you want to borrow?: ");
            bookId = scanner.nextLine();
            valid = bookIds.contains(bookId);
        }

        String memberId = "";
        valid = true;
        while (memberId.isBlank() || memberId.isEmpty() || !valid) {
            if (!valid) {
                System.out.println("That member doesn't exist in our records");
            }
            System.out.print("And what is your member id?: ");
            memberId = scanner.nextLine();
            valid = memberIds.contains(memberId);
        }
        
        // TODO: reject if already borrowed; mark unavailable; put borrowRecords

        if (borrowRecords.containsValue(bookId)) {
            System.out.println("That book is already being borrowed. Sorry.");
            return;
        }

        // set availablility to false
        Book book = findBookById(bookId);
        book.setAvailable(false);

        borrowRecords.put(memberId, bookId);

        
        // TODO: add BorrowRecord to history; bump borrowFrequency; success message

        borrowHistory.add(new BorrowRecord(memberId, bookId, LocalDate.now()));
    ...
    public void returnBook() {
        ...
        while(bookId.isBlank() || bookId.isEmpty() || !valid) {
            ...
            System.out.print("Hello, what is the book id that you want to return?: ");
            bookId = scanner.nextLine();
            valid = bookIds.contains(bookId);
        }

        // Check if the book was actually checked out
        if (!borrowRecords.containsKey(bookId)) {
